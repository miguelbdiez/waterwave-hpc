...
bool config_load(const char* filename, Config* config) {
    ...
    char* env;
    if ((env = getenv("OPENMP_THREADS")) != NULL) {    
        config->openmp_threads = (size_t)atoi(env);
    }
...
void config_print(const Config* config) {
    ...
    printf("  Workers pthreads:     %zu\n", config->threads);                        
    printf("  Threads OpenMP:       %zu (por worker)\n", config->openmp_threads);  
    printf("  Total hilos:          %zu\n", config->threads * config->openmp_threads); 
...
static void parallel_worker(void* arg) {
    ...
    // Si hay OpenMP configurado, usarlo dentro de este pthread
    if (g_config.openmp_threads > 1) {
        // Paralelizar con OpenMP el rango asignado a este pthread
        #pragma omp parallel for num_threads(g_config.openmp_threads)
        ...
        // Sin OpenMP, ejecutar secuencialmente
        for (size_t i = data->x_start; i < data->x_end; i++) {
            for (size_t j = data->y_start; j < data->y_end; j++) {
                data->callback(i, j, data->callback_data);
            }
