...
bool config_load(const char* filename, Config* config) {
    ...
    if ((env = getenv("OMP_NUM_THREADS")) != NULL) {    
        config->openmp_threads = (uint32_t)atoi(env);
    }
...
void config_print(const Config* config) {
    ...
    printf("  Threads OpenMP:       %u (por proceso MPI)\n", config->openmp_threads);    
...
void mpi_state_init(int* argc, char*** argv) {
    // Inicializar MPI con soporte de hilos
    int provided;
    MPI_CHECK(MPI_Init_thread(argc, argv, MPI_THREAD_FUNNELED, &provided));
    
    if (provided < MPI_THREAD_FUNNELED) {
        fprintf(stderr, "Advertencia: MPI no provee soporte adecuado de hilos\n");
    }
    ...
    LOG("MPI inicializado (%d procesos totales, soporte hilos: %d)\n", 
        g_mpi.proc_count, provided);
}
...
void simulation_update_in_x_direction(SimulationState* s) {
    ...
    #pragma omp parallel for collapse(2) if(g_config.openmp_threads > 1)
...
int main(int argc, char** argv) {
 ...
 // Validar argumentos
    ...
    // Configurar número de threads OpenMP por proceso MPI
    if (g_config.openmp_threads > 0) {
